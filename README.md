...
- ***type*** (string) - default: `payment`. Possible value: `payment, swap`. Indicate which app will behave. If value `payment`, Interface will become payment, `receiveAddr` is required. If value `swap`, Interface will be swap, `receiveAddr` must be blank or `self`, other cases will cause error.
